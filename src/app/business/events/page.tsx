...
export default function BusinessEventsPage() {
  ...
  const [advertisedEventIds, setAdvertisedEventIds] = useState<string[]>([
    'evt_static_1',
  ]);
  ...
  const toggleAdvertiseEvent = (eventId: string) => {
    setAdvertisedEventIds((prev) =>
  ...
  const isEventAdvertised = (eventId: string) =>
    advertisedEventIds.includes(eventId);

  const getLocationIcon = (type: string) => {
    if (type === 'virtual')
      return <Globe className="w-4 h-4 text-neutral-400" />;
    if (type === 'hybrid')
      return <Users2 className="w-4 h-4 text-neutral-400" />;
  ...
  if (view === 'show' && selectedEvent) {
    return (
      <div className="p-4 sm:p-6">
        <ShowEvent
          event={selectedEvent}
          isAdvertised={isEventAdvertised(selectedEvent.id)}
          onToggleAdvertise={toggleAdvertiseEvent}
          onClose={() => {
            setView('list');
            setSelectedEvent(null);
          }}
      ...
      <div className="mx-auto w-full max-w-6xl">
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button
            ...
            variant="outline"
            className="w-full border border-neutral-300 px-4 py-2 text-neutral-700 font-medium rounded-xl hover:bg-neutral-50 transition-colors sm:w-auto"
          >
            ...
            >
              ...
              <CardContent className="pt-0 space-y-4">
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  <Button
                    variant="outline"
                    className="text-sm border border-neutral-300 hover:bg-neutral-50 rounded-lg"
                    onClick={(e) => {
                      e.stopPropagation();
                      toggleAdvertiseEvent(event.id);
                    }}
                  >
                    {isEventAdvertised(event.id) ? 'Stop Ad' : 'Advertise'}
                  </Button>
                  <Button
                    variant="outline"
                    className="text-sm border border-neutral-300 hover:bg-neutral-50 rounded-lg flex-1 sm:flex-none"
                    onClick={(e) => {
                      e.stopPropagation();
                  ...
                  <Button
                    className="text-sm bg-[#ff5f6d] hover:bg-[#ff5f6d]/90 text-white rounded-lg flex-1 sm:flex-none"
                    onClick={(e) => {
                      e.stopPropagation();

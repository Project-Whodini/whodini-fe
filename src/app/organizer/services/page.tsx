...
export default function OrganizerServicesPage() {
  ...
  const [filterStatus, setFilterStatus] = useState<
    'all' | 'available' | 'booked' | 'unavailable'
  >('all');
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-neutral-600 mt-1">
              Manage service providers for{' '}
              <span className="font-semibold">{organizerName}</span> events
            </p>
          </div>
          <Button
        ...
        <div className="mb-6">
          <div className="flex flex-wrap gap-2">
            {(['all', 'available', 'booked', 'unavailable'] as const).map(
              (status) => (
                <Button
                  key={status}
                  type="button"
          ...
          <Card className="border border-neutral-200 rounded-xl bg-white shadow-sm">
            <CardContent className="pt-12 pb-12 text-center">
              ...
              <CardDescription className="text-neutral-600 mb-6">
                Add your first service provider to manage event logistics and
                bookings.
              </CardDescription>
              <Button
                onClick={() => setView('create')}
                ...
                >
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      ...
                      <Badge
                        className={`capitalize shrink-0 ${getStatusColor()}`}
                      >
                        {service.status}
                      </Badge>
                    </div>
                  ...
                  <CardContent className="pt-0 space-y-3">
                    ...
                    {service.eventIds.length > 0 && (
                      <div className="flex items-center gap-2">
                        <Badge variant="outline" className="text-xs">
                          {service.eventIds.length} event
                          {service.eventIds.length > 1 ? 's' : ''}
                        </Badge>
                      </div>
                    )}
                  </CardContent>

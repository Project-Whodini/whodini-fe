...
const ACCOUNT_TYPES = [
  { id: "personal", label: "Personal Account", initials: "PA" },
  { id: "business", label: "Business/Brand", initials: "BB" },
  { id: "community", label: "Community/Organization", initials: "CO" },
  { id: "event", label: "Event Organizer", initials: "EO" },
  { id: "agency", label: "Agency", initials: "AG" },
] as const;
...
const NAV_ITEMS = [
  ...
  { label: "Organizer", href: "/organizer", icon: Ticket },
...
export default function SideBar() {
  ...
  const selectedAccount = ACCOUNT_TYPES.find((a) => a.id === accountTypeId) ?? ACCOUNT_TYPES[0];
  ...
  if (!mounted) {
    return (
      <aside className="h-screen w-64 bg-[#ff5f6d] transition-[width] duration-[380ms] ease-[cubic-bezier(0.32,0.72,0,1)] overflow-hidden" />
    );
  }

  return (
    <aside
      className={`h-screen flex flex-col items-center py-8 shadow-xl rounded-r-2xl bg-gradient-to-br from-[#ff5f6d] to-[#ffc371] overflow-hidden transition-[width] duration-[380ms] ease-[cubic-bezier(0.32,0.72,0,1)] ${
        expanded ? "w-64" : "w-[4.5rem]"
      }`}
    >
      <div
        className={`mb-8 flex flex-col items-center w-full ${expanded ? "px-4" : "px-2"}`}
        ref={dropdownRef}
      >
        ...
        <div className="relative w-full flex justify-center">
          ...
          >
            ...
            {expanded && (
              <>
                <span className="flex-1 min-w-0 text-base font-medium text-neutral-900 truncate">
          ...
          {dropdownOpen && (
            <ul
              ...
              className={`absolute py-1.5 bg-white rounded-xl shadow-lg ring-1 ring-black/10 overflow-hidden z-50 origin-top transition-all duration-200 ease-out ${
                expanded ? "left-0 right-0 top-full mt-1.5" : "left-full top-0 ml-1.5 min-w-[12rem]"
              } ${
                    ...
                    >
                      <span
                        className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold ${
                          isSelected ? "bg-white/25 text-white" : "bg-neutral-200 text-neutral-600"
                        }`}
                      >
                        {type.initials}
                      </span>
                      <span className="flex-1 min-w-0 text-sm truncate">{type.label}</span>
                    </button>
                  </li>
                );
              })}
      ...
      <nav className={`flex flex-col gap-2 w-full ${expanded ? "px-4" : "px-2 items-center"}`}>
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              ...
              className={`text-white font-medium rounded-xl hover:bg-white/10 transition flex items-center gap-3 ${
                expanded ? "py-2 px-4" : "w-10 h-10 justify-center p-0"
              }`}
            >
      ...
      <div className={`w-full flex flex-col items-center gap-2 ${expanded ? "px-4" : "px-2"}`}>
        <button
          type="button"
